...
- **依天數自動計算數量**：襪子、上衣、內褲、隱形眼鏡（含備用）會依你設定的行程天數自動顯示需要帶幾份；衣物數量只顯示純數字，不另外寫出天數
- **打勾/打叉追蹤進度**：每一項目點一下是勾（帶）、點兩下是叉（這次不帶）、點三下清除；勾和叉都算進度，不再區分「必要品/非必需品」
- **證件流程**：預設國內線（駕照錢包），可切換國際線（護照）；按下「① 帶了」後，國際線／國內線切換鈕會自動隱藏，按「② 已收好」才算完成
- **手提行李／托運開關**：Carry-on 手提行李、Checked Baggage 托運行李各自有獨立開關；國內線預設「手提行李開、托運關」，國際線預設「兩者都開」，切換國內/國際線會自動套用對應預設值，之後仍可手動調整。只要托運開關是開的，衣物類（上衣、襪子、內褲、褲子、外套、鞋子 or 拖鞋）就會自動歸到 Checked Baggage；托運關掉時衣物則歸到 Carry-on
- **編輯模式**：可以在每個分類底下自行新增項目，新增時可選擇是否要「不顯示 / 固定數量 / 跟天數走」；已新增的項目可以改名或刪除。原始清單項目維持固定，不能刪改。衣物可在編輯時調整數量顯示方式；新增項目的數量模式也可自行指定。新增的項目會存起來，就算按「開始新的旅程」重置勾選狀態，項目本身也不會消失
- **項目可拖曳排序**：編輯模式下，Personal Item 和 Carry-on 底下每個項目右側會出現拖曳把手，按住上下拖曳即可調整同一分類內的順序（滑鼠與觸控皆可），順序會記住；分類本身順序固定，不能拖動
- **出發前準備**：新增第 05 區塊，作為純提醒清單，預設放清冰箱、倒垃圾、電子產品充電、關燈與電器、檢查證件與航班等項目；不計入主行李完成率
- **狀態提示**：頂部會依完成度切換成「🐹 今天要出發了！還有 n 項沒帶。」或「🐹 行李整理完成！ ✈️」；主完成率只計 01 到 04，不含 X 與 05
