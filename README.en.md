...
- **Edit mode**: add your own items under any category, and choose whether each new item should stay hidden, use a fixed quantity, or scale with trip length; added items can be renamed or deleted. Built-in items stay fixed and can't be edited or removed. Clothing items can still toggle quantity display/editing. Added items are saved permanently — even tapping "Start a new trip" (which resets checkmarks) won't remove them
...
- **Pre-departure prep**: a new Section 05 used as a reminder-only checklist (clear the fridge, take out trash, charge electronics, turn off lights/appliances, check documents and flight details); it does not count toward the main packing completion rate
- **Status prompts**: the top headline switches between "🐹 Today is the day! N items are still missing." and "🐹 Packing complete! ✈️"; the main completion rate counts only Sections 01 to 04, excluding X items and Section 05
